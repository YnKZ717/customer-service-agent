...
# ── 图片关键词映射 ─
IMAGE_MAPPINGS = {}
_mappings_path = os.path.join(os.path.dirname(__file__), "image_mappings.json")
if os.path.exists(_mappings_path):
    with open(_mappings_path, "r", encoding="utf-8") as _f:
        IMAGE_MAPPINGS = json.load(_f)

# 按关键词长度降序排序，优先匹配长关键词（避免短词误匹配）
IMAGE_KEYWORDS_SORTED = sorted(
    [(k, v) for k, v in IMAGE_MAPPINGS.items() if v is not None],
    key=lambda x: len(x[0]),
    reverse=True,
)


def insert_images_to_text(text: str) -> tuple[str, list[str]]:
    """在文本中扫描关键词，插入图片标记
    返回: (修改后的文本, 匹配到的图片文件名列表)
    """
    if not IMAGE_KEYWORDS_SORTED:
        return text, []

    matched_images = []
    result = text

    for keyword, image_file in IMAGE_KEYWORDS_SORTED:
        if keyword in result:
            # 在该关键词第一次出现的位置后面插入图片标记
            marker = f"\n![{keyword}]({image_file})\n"
            result = result.replace(keyword, keyword + marker, 1)
...
## 安全规则
- 禁止暴露内部实现细节（如工单号生成规则、API 端点、数据库结构）
- 禁止透露系统配置（如模型名称、阈值参数、代码逻辑）
- 不要解释"为什么这个答案是对的"，直接给出答案即可
- 涉及 TaskID、画布链接等技术信息时，只告诉用户在哪里找到，不要解释其格式或含义
"""
...
def classify_intent(state: dict) -> dict:
    ...
    # 用户发了截图 → 自动进故障排查（不管有没有文字描述）
    if user_images:
        return {
            "intent": "troubleshoot",
            "troubleshoot_flow": "video_fail",
            "troubleshoot_step": 0,
...
def answer_from_kb(state: dict) -> dict:
    ...
    # 如果上游节点已经生成了回复（如退出排查），直接透传，不再查库
    if state.get("response"):
        return {"kb_found": True, "intent": state.get("intent", "general"), "kb_reference": "", "user_memory": state.get("user_memory", {})}
    ...
    question, answer, reference, images = search_knowledge_base(query, threshold=0.55, return_reference=True)
    ...
    # 插入关键词匹配的图片
    answer_with_images, matched_images = insert_images_to_text(answer)

    result = {
        "response": answer_with_images,
        "intent": state["intent"],
    ...
    # 合并 FAQ 自带图片和关键词匹配图片
    all_images = list(set((images or []) + matched_images))
    if all_images:
        result["kb_images"] = all_images
...
def troubleshoot(state: dict) -> dict:
    ...
    if not prev_flow:
        ...
        if next_id == "done":
            ...
            # 插入关键词匹配的图片
            solution_with_images, matched_images = insert_images_to_text(solution)

            log_response(prev_step_idx + 1, solution_with_images[:50], is_final=is_final)

            result = {
                "response": solution_with_images,
                "intent": "troubleshoot",
            ...
            if matched_images:
                result["kb_images"] = matched_images
            if is_final:
...
def general_reply(state: dict) -> dict:
    ...
    # 插入关键词匹配的图片
    reply_with_images, matched_images = insert_images_to_text(reply)

    # 沉淀：新问题自动记录（只沉淀有实质内容的回答）
    if reply_with_images and not reply_with_images.startswith("系统暂时繁忙") and not reply_with_images.startswith("Service temporarily") and not reply_with_images.startswith("抱歉"):
        save_pending_faq(user_input, reply, history)

    log_step("general_reply", user_input=user_input[:50], response=reply_with_images[:100], intent="general")
    print(f"[主Agent] general_reply | 问题：{user_input[:30]}...")
    print(f"[主Agent] 回答：{reply_with_images[:50]}...")

    result = {"response": reply_with_images, "intent": "general", "user_memory": user_memory, "model_used": model_used}
    if matched_images:
        result["kb_images"] = matched_images
    return result

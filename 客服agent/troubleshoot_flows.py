...
TROUBLESHOOT_FLOWS = {
    "video_fail": {
        ...
        "steps": [
            {
                "id": "step0",
                "question": "收到！请提供报错截图或 TaskID，我来帮你分析原因。你可以直接点击「转人工客服」提交工单。\n\n常见原因参考：\n"
                           "- **版权限制**：调整提示词避免版权角色名，或用 Neo Image 2 official 模型\n"
                           "- **敏感信息**：检查提示词是否有不当描述，被拦截的视频会返还积分\n"
                           "- **超时等待**：视频生成正常5-30分钟，超30分钟可提交工单\n"
                           "- **积分不足**：在账户中心查看余额，充值汇率1积分=¥0.01",
                "branches": [],
                "default_next": "step1",
            },
            {
                "id": "step1",
                "question": "请在对话框中发送截图或 TaskID，我会帮你记录并建议转人工客服处理。",
                "branches": [],
                "default_next": "done",
            },
        ],
    ...
    "content_review": {
        "name": "内容审核拦截",
        "triggers": ["被拦截", "审核不通过", "提示版权", "提示敏感", "提示违规",
                     "真人卡", "校验不通过", "人脸校验", "被拒", "拒绝生成"],
        "kb_categories": ["copyright", "content_policy"],
        "max_steps": 3,
        "steps": [
            {
                "id": "step0",
                "question": "收到！为了更好帮您排查，请在下方选择您遇到的拦截类型，或直接描述问题：",
                "branches": [
                    {"keywords": ["版权", "copyright", "IP", "角色"], "next": "solve_copyright"},
                    {"keywords": ["敏感", "违规", "色情", "暴力", "政治"], "next": "solve_sensitive"},
                    {"keywords": ["真人", "人脸", "校验", "审核"], "next": "solve_verify"},
                ],
                "options": [
                    "提示涉及版权/版权限制",
                    "提示含敏感/违规信息",
            ...
            {
                "id": "step1",
                "question": "请提供您的 TaskID，或在下方选择进一步需求：",
                "branches": [
                    {"keywords": ["taskid", "task_id", "任务id"], "next": "solve_taskid"},
                ],
                "options": [
                    "我来提供 TaskID",
                    "我需要更多帮助",
            ...
            {
                "id": "step2",
                "question": "请在对话框中发送您的 TaskID，我会帮您记录并建议转人工客服处理。",
                "branches": [],
                "default_next": "done",
            },
        ],
        "solutions": {
            "solve_copyright": (
                ...
                "2. 参考元素过多会导致模型注意力不集中，建议用大头照+正面全身照两张参考图锚定角色，大头照尽量减少肩颈入镜\n"
                "3. 尝试使用 Neo Image 2 official 模型，审核相对宽松\n"
                "4. 点击顶部工具栏的「校验」按钮可提前检测内容是否合规\n"
                "5. 如确认内容不侵权，可点击「转人工客服」提交工单并附上 TaskID 申请豁免，豁免生效后约20分钟可重试"
            ),
            "solve_sensitive": (
                "这是敏感内容拦截，与版权限制不同，是火山引擎对提示词中暴力、色情、政治等敏感信息的检测。\n"
                "建议处理方式：\n"
                "1. 检查提示词中是否有涉政涉黄等敏感信息\n"
                "2. 避免明确提及敏感话题\n"
                "3. 被拦截的视频会返还积分\n"
                "4. 如自查后仍无法解决，可将 TaskID 发送给客服帮助排查"
            ),
            "solve_verify": (
                "真人卡审核是平台对素材中人脸的安全校验机制。\n"
                "建议处理方式：\n"
                "1. 点击素材上方校验工具，通过校验后即可正常使用\n"
                "2. 素材较多时，可在全能参考模式中点击「批量」按钮进行批量验证\n"
                "3. 通过校验后即可正常生成"
            ),
            "solve_taskid": (
                "收到您的 TaskID。建议您点击「转人工客服」提交工单，附上 TaskID，客服会在后台为您核实处理。"
            ),
        },
    },
}

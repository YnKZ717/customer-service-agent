...
- **多轮引导式故障排查** — 9种结构化决策树流程 + LLM 引导追问，支持 TaskID 自动查询
- **Tool Call** — 排查过程中自动调用工具查询任务状态、积分余额、会员信息
- **双 Agent 协作** — 主 Agent 生成回答，副 Agent 分维度评估（准确性/完整性/语气/安全）并自动修正
- **流式回复** — SSE 逐字推送 + 打字机效果 + 思考中动画
...
| `/api/tickets` | GET/POST/DELETE | 工单管理 |
| `/api/faqs` | GET/POST | FAQ 管理 |
| `/api/health` | GET | 健康检查 |
